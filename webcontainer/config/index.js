...
buildWithoutHTML &&
	typeof buildWithoutHTML === 'string' &&
	(buildWithoutHTML = [
		buildWithoutHTML
	]);
...
let { entries: hashEntries } = buildEntry({ buildPath, buildWithoutHTML, hash: true });
let { entries } = buildEntry({ buildPath, buildWithoutHTML, hash: false });
